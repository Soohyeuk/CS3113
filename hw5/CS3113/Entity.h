...
enum AIState      { WALKING, IDLE, FOLLOWING, FLYING         };
enum PlayerAnimState { PLAYER_IDLE, PLAYER_RUN  };
...
private:
    ...
    bool mUsesPlayerAnimationSet = false;
    PlayerAnimState mPlayerAnimState = PLAYER_IDLE;

    Vector2 mIdleSpriteSheetDimensions;
    Vector2 mRunSpriteSheetDimensions;
    std::vector<int> mIdleAnimationIndices;
    std::vector<int> mRunAnimationIndices;
...
public:
    ...
    void configurePlayerAnimationSet(const char *idleTexturePath,
    ...
    bool checkCollisionWith(Entity *other) const { return isColliding(other); }
    void resolveAgainst(Entity *other);
