...
private:
    ...
    std::unordered_set<unsigned int> mSolidTiles; // 1-based atlas indices that block movement
    unsigned int mFloorTile;  // optional base tile drawn beneath every cell (0 = disabled)
...
public:
    Map(int mapColumns, int mapRows, unsigned int *levelData,
        ...
        std::unordered_set<unsigned int> solidTiles = { 2 },
        unsigned int floorTile = 0);
    ~Map();

    void build();
    void render();
    bool isSolidTileAt(Vector2 position, float *xOverlap, float *yOverlap);
    unsigned int getTileAt(Vector2 position) const;
