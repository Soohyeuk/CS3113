...
void Map::render()
{
    ...
    for (int row = 0; row < mMapRows; row++)
    {
        ...
        {
            // Get the logical tile index at the current row and column
            int tile = mLevelData[row * mMapColumns + col];

            // If the tile index is 0, we do not draw anything
            if (tile == 0) continue;

            // Remap legacy level IDs to the world_tileset (2x3):
            // 2 -> 1 (grass), 3 -> 3 (unchanged),
            // 4 -> 2 on exposed top, 4 elsewhere (wall body)
            int atlasTile = tile;
            if (tile == 2)
            {
                atlasTile = 1;
            ...
            {
                bool hasNoTileAbove = (row == 0) || (mLevelData[(row - 1) * mMapColumns + col] == 0);
                atlasTile = hasNoTileAbove ? 2 : 4;
            }

            if (atlasTile < 1 || atlasTile > static_cast<int>(mTextureAreas.size())) continue;
            ...
            DrawTexturePro(
                mTextureAtlas,
                mTextureAreas[atlasTile - 1], // -1 because tile indices start at 1
                destinationArea,
                {0.0f, 0.0f}, // origin
                0.0f,         // rotation
                WHITE         // tint
